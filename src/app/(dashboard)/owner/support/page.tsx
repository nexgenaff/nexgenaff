...
export default function OwnerSupportPage() {
  ...
  useEffect(() => {
    ...
    const refreshSupport = async () => {
      try {
        ...
        setUsers(data.users || [])
        setSelectedId((currentId) => currentId || data.users[0]?.id || null)
        setError('')
      } catch (loadError) {
  ...
  return (
    <section className="flex h-[calc(100dvh-3rem)] min-h-0 flex-col space-y-5">
