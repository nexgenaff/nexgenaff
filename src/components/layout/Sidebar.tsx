      ...
      <nav className={`relative z-10 min-h-0 flex-1 overflow-y-auto overscroll-contain ${isMobile ? 'space-y-4 px-3 py-4' : 'space-y-2 px-2 py-2'}`}>
        {menuGroups.map((group) => group.items.length > 0 && (
          <div key={group.label} className={`space-y-1.5 ${isMobile ? 'pb-2' : ''}`}>
            {(!collapsed || isMobile) && (
              <div className={`${isMobile ? 'px-3 py-2 text-[11px] font-bold uppercase tracking-[0.16em] text-slate-600 dark:text-slate-300 bg-slate-400/5 dark:bg-white/5 rounded-lg border border-slate-200/30 dark:border-white/5' : 'px-2 pt-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500 first:pt-0 dark:text-slate-400'}`}>
                {group.label}
              </div>
            )}
            {group.items.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`group flex items-center ${collapsed && !isMobile ? 'justify-center' : 'gap-3'} ${isMobile ? 'min-h-12 rounded-lg px-4 py-3' : 'rounded-lg px-2 py-1.5'} border transition-all duration-200 ${
                    isActive
                      ? isMobile 
                        ? 'border-cyan-400/40 bg-cyan-500/15 font-medium text-slate-900 dark:text-slate-50 shadow-sm' 
                        : 'border-cyan-500/25 bg-cyan-500/[0.12] font-medium text-slate-800 dark:text-slate-50'
                      : isMobile
                        ? 'border-slate-300/30 text-slate-700 hover:border-slate-400/50 hover:bg-slate-200/40 dark:border-white/8 dark:text-slate-300 dark:hover:border-white/15 dark:hover:bg-white/[0.08]'
